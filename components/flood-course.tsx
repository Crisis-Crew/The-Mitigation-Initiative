...
import { floodCourse } from "@/lib/flood-course";
...
export default function FloodCourse({
  userId,
}: {
  userId: string | undefined;
}) {
  const questions = floodCourse.map((item) => item.question);
  const real_answers = floodCourse.map((item) => item.answer);
  const [answers, setAnswers] = useState(
    Array.from({ length: floodCourse.length }),
  );
  const chartData = [
    {
      answer: "correct",
    ...
    <>
      <Dialog open={submitted} onOpenChange={setSubmitted}>
        <DialogContent className="min-w-75 max-w-80 max-h-120 overflow-scroll">
          ...
          <Link href={`/game/flood/?id=${userId}`}>
      ...
      <section
        className="section pt-32! pb-8! flex justify-center bg-center bg-cover"
        style={{
          backgroundImage: "url('/assets/backgrounds/flood.png')",
        }}
      >
        <div
        ...
        >
          ...
          <div className="relative z-10 p-6 lg:p-12 space-y-4">
            ...
            <p className="text-sm leading-tight text-muted-foreground max-w-2xl">
              A flood is a natural disaster that occurs when excess water
              submerges land that is usually dry. This can happen due to heavy
              rainfall, overflowing rivers, dam failures, or coastal storms.
              <br />
              <br />
              This quiz tests your understanding of essential flood safety
              measures — what to do before, during, and after a flood — helping
              you stay prepared and reduce risk during emergencies.
            </p>
          </div>
        </div>
      </section>
